					...
					<div id="box-men">
								...
								<div class="box-food">
									<?php
										...
										$sql="SELECT * FROM fglist WHERE kategori=1";
										if(isset($_POST['sorting'])) {
											$value= $_POST['sorting'];
											if($value == 'alfabet') {
    											// query to get all Fitzgerald records
   												$sql="SELECT * FROM fglist WHERE kategori=1 ORDER BY nama";
											}
												elseif($value == 'waktu') {
    												// query to get all Herring records
    												$sql="SELECT * FROM fglist WHERE kategori=1 ORDER BY id DESC";
													}
											...
											echo'<label class="box text-center">
												...
												<div class="centr">
													<form id="pilih_mode" method="post" action="'.site_url().'/choosefg/detailfg">
														<button type="submit" class="submit btn btn-primary" name="detail" value="'.$row['nama'].'">Details</button>
													</form>
												</div>
												</label>';
											echo'		</div>
