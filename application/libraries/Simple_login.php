 ...
 class Simple_login {
     ...
     */
     public function login($username, $password) {
         ...
         if($query->num_rows() == 1) {
			 ...
			 $rows  = $this->CI->db->query('SELECT avatar FROM users where username = "'.$username.'"');
             $admin     = $row->row();
			 $admins = $rows->row();
             $id   = $admin->id_user;
             ...
             $this->CI->session->set_flashdata('sukses','WRONG USERNAME/PASSWORD. TRY AGAIN ... ');
	  ...
	  public function flogin($username, $password) {
         ...
         //cek username dan password
         $query = $this->CI->db->get_where('fglist',array('username'=>$username,'password' => md5($password)));
 
         if($query->num_rows() == 1) {
             //ambil data user berdasar username
             $row  = $this->CI->db->query('SELECT id FROM fglist where username = "'.$username.'"');
			 $rows  = $this->CI->db->query('SELECT avatar FROM fglist where username = "'.$username.'"');
             $admin     = $row->row();
			 $admins = $rows->row();
             $id   = $admin->id;
			 $avatar = $admins->avatar;
     ...
     public function cek_login() {
         ...
         if($this->CI->session->userdata('username') == '') {
             ...
             //set notifikasi
             $this->CI->session->set_flashdata('sukses','You have not login...');
	...
	public function cek_loginfg() {
     ...
     public function logout() {
         ...
         $this->CI->session->set_flashdata('sukses','You have logout successfully');
		 $this->CI->session->unset_userdata('avatar');
